...
public final class AcademicPhysiqueEvolutionEvents {

    private AcademicPhysiqueEvolutionEvents() {}

    private static final String TAG_ROOT = "ascension_academic_soul_evolution";
    private static final String TAG_TIMER = "evolution_timer";

    private static final long TICKS_PER_MINUTE = 1200L;
    private static final long TICKS_REQUIRED = 1L * TICKS_PER_MINUTE; // 20 minutes
    ...
    public static void onPlayerTick(PlayerTickEvent.Post event) {
        ...
        boolean hasAcademicSpirit = PhysiqueEvolutionEventUtil.hasPhysique(entityData, ModPhysiques.ACADEMIC_SPIRIT.getId());


        if (!hasAcademicSpirit) return;
        ...
        boolean hasRealm = soulData.getMajorRealm() >= 2;
        if (hasAcademicSpirit && hasRealm) {
            ...
            if (timer >= TICKS_REQUIRED) {
                PhysiqueEvolutionHelper.tryEvolveInto(player, entityData, ModPhysiques.PRODIGIOUS_SOUL.getId());
                clearTag(player);
            }
        } else {
            clearTag(player);
    ...
    public static void onPhysiqueChange(PhysiqueChangeEvent.Post event) {
        ...
        if (!ModPhysiques.ACADEMIC_SPIRIT.getId().equals(newPhysique)) {
            clearTag(player);
        }
    }

    private static boolean isScholarlyTechnique(ResourceLocation technique) {
        if (technique == null) return false;
        return ModTechniques.SCHOLARLY_SOUL_TECHNIQUE.getId().equals(technique);
